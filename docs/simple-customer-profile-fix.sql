-- Simple Customer Profile Fix - Run this to fix the customer authentication
-- Run this SQL in your Supabase Dashboard > SQL Editor

-- Get the invitation data and create the missing profile and customer record
DO $$
DECLARE
    user_id_val UUID;
    shop_id_val UUID;
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM public.profiles WHERE id = user_id_val) THEN
        INSERT INTO public.profiles (
            id, first_name, last_name, email, role, shop_id, created_at, updated_at
        ) VALUES (
            user_id_val, invitation_data.first_name, invitation_data.last_name, 
            invitation_data.email, 'customer', shop_id_val, now(), now()
        );
    ...
    -- Create the customer record if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM public.customers WHERE id = user_id_val) THEN
        INSERT INTO public.customers (
            id, shop_id, first_name, last_name, email, phone, customer_type, 
            date_of_birth, age_verified, created_at, updated_at
        ) VALUES (
            user_id_val, shop_id_val, invitation_data.first_name, invitation_data.last_name,
            invitation_data.email, invitation_data.phone, 
            COALESCE(invitation_data.customer_type, 'regular'),
            invitation_data.date_of_birth, invitation_data.age_verified, now(), now()
        );
...
-- Final status
SELECT 'Customer profile fix completed! User should now be able to sign in.' as status;
